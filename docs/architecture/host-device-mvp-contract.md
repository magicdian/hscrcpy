...
* Host shutdown is cancellation-driven across startup, capture, render, and cleanup. Ctrl+C/SIGINT must not wait for the next IDR or the next device payload to make progress; long-running host phases must use bounded waits or cancellation-aware I/O and then call the selected route cleanup path.
* The cancellation contract must remain portable across macOS, Linux, and Windows. Platform-specific readiness mechanisms such as `select`, `poll`, `epoll`, kqueue, or Windows overlapped I/O may be used inside adapters, but the host runtime boundary should expose route-neutral "wait for video/control/cancellation" semantics.
...
| Host shutdown requested while route startup or video ingress is blocked | host during startup/capture/render | Break the wait through cancellation-aware or bounded I/O, call selected route cleanup, and report interrupted shutdown context | `host_shutdown_requested` |
...
* Manual lifecycle and stream check for the official `uitest` route:
  ...
  * Ctrl+C/SIGINT during route startup, while no IDR is received, while only pre-IDR H.264 units are received, and while no stream data is available exits through graceful selected-route cleanup without waiting for an eventual device frame
